    ...
    def predict_with_correction(
        ...
        if true_solver_fn is not None and self.correction_policy.should_correct(self._step):
            ...
            if isinstance(surrogate_output, dict) and isinstance(true_output, dict):
                ...
                main_key = (
                    "output"
                    if "output" in surrogate_output
                    else next(k for k in surrogate_output if not k.endswith("_std"))
                )
                error = torch.mean(
                    (true_output.to(surrogate_output[main_key].device) - surrogate_output[main_key])
                    ** 2
                ).item()
